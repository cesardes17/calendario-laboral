...
import { WorkCycle } from '../domain/work-cycle';
import { EmploymentStatus } from '../domain/employment-status';
import { ContractStartDate } from '../domain/contract-start-date';
import { CycleOffset } from '../domain/cycle-offset';
import { WorkingHours } from '../domain/working-hours';
import { AnnualContractHours } from '../domain/annual-contract-hours';
...
export function validateCompleteConfiguration(
  ...
  if (!config.employmentStatus) {
    fields.push({
      ...
      navigateTo: 'employment-status-section',
    });
  } else {
    ...
    if (config.employmentStatus.isStartedThisYear()) {
      if (!config.contractStartDate) {
        ...
        fields.push({
          field: 'employmentStatus',
          status: FieldStatus.COMPLETE,
          message: `Empezó el ${config.contractStartDate.getFormattedDate()}`,
        });
        completedSections++;
      }
    } else {
  ...
  if (!config.workingHours) {
    fields.push({
      ...
      navigateTo: 'working-hours-section',
    });
  } else {
    fields.push({
      field: 'workingHours',
      status: FieldStatus.COMPLETE,
      message: `L-V: ${config.workingHours.weekday}h, Sáb: ${config.workingHours.saturday}h, Dom: ${config.workingHours.sunday}h, Festivo: ${config.workingHours.holiday}h`,
    });
    completedSections++;
  }
  ...
  if (!config.annualContractHours) {
    fields.push({
      ...
      navigateTo: 'annual-contract-hours-section',
    });
  } else {
    fields.push({
      field: 'annualContractHours',
      status: FieldStatus.COMPLETE,
      message: `${config.annualContractHours.value} horas/año`,
    });
    completedSections++;
  }
